...
exports.addAmenities = async (payloadData, res) => {
    ...
    if (payloadData && payloadData.files && payloadData.files.image) {
        ...
        let key = S3.genKeyFromFilename(`amenities`, image.name || 'jpg', []);
        pararms["image"] = await S3.uploadFile(key, image.data, { publicRead: true, mimeType: image.mimetype }, 1);
    }
    const data = await utils.saveData(Amenities, pararms);
...
exports.updateAmenities = async (payloadData, res) => {
    ...
    const data = await utils.updateData(Amenities, { _id: pararms.id }, pararms);
    return sendSuccessMessage('success', data, res);
};
exports.deleteAmenities= async (payloadData, res) => {
    const pararms = payloadData.query;
    await utils.updateData(Amenities, { _id: pararms.id}, [
...
exports.getAllAmenities = async (payloadData, res) => {
    ...
    let data = await utils.getData(Amenities, {
        query:query,
        sort:{_id:-1},
    ...
    const count  = await utils.countDocuments(Amenities,query);
...
exports.getAmenitiesById = async (payloadData, res) => {
    const pararms = payloadData.query;
    const data = await utils.getData(Amenities, {
