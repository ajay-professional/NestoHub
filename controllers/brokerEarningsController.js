...
const { BrokerEarnings, BoughtProperty, Claim, Visit, LoanQueryDetails, Invoice } = require('../models');
...
exports.getAllBrokerEarningsForMobile = async (payloadData, res) => {
  ...
  let visitEarnings = await utils.aggregateData(Claim, [
    ...
    const visitCounts = await utils.aggregateData(Visit, [
        {
          $match: {
            brokerId: mongoose.Types.ObjectId(pararms.brokerId),
...
exports.getBrokerEarningsByBrokerId = async (payloadData, res) => {
  const pararms = payloadData.query;
  let data;
  let result=[];
  if (pararms.type == 'total') {
    ...
    let query = { isDeleted: false, brokerId: pararms.brokerId, paidTo: "broker", status: "settled" };
    if (pararms.search) {
      query['$or'] = [
    ...
    data = await utils.getData(Invoice, {
      query: query,
      sort: { _id: -1 },
    ...
    for (let datum of data) {
      result.push({
        claimId: datum.claimId._id,
        transactionId: datum.transactionId,
        amount: datum.invoiceAmount,
        date: datum.paymentDate
      });
    }
  }
  else if (pararms.type == 'upcoming') {
    const populates = ['brokerId', 'builderId', {
      path: 'claimId',
      model: 'claim',
      populate: [
    ...
    let query = { isDeleted: false, brokerId: pararms.brokerId, paidTo: "broker", status: "pending" };
    if (pararms.search) {
      query['$or'] = [
        { invoiceAmount: { $regex: pararms.search, $options: "i" } },
        { claimId: { $regex: pararms.search, $options: "i" } },
        { date: { $regex: pararms.search, $options: "i" } },
        { claimStatus: { $regex: pararms.search, $options: "i" } },
      ];
    }
    data = await utils.getData(Invoice, {
      query: query,
    ...
    for (let datum of data) {
      result.push({
        claimId: datum.claimId._id,
        date: datum.claimId.date,
        amount: datum.invoiceAmount,
        status: datum.claimId.claimStatus
      });
    }
  }
  else if (pararms.type == 'visit') {
    const populates = ["brokerId", "builderId", "customerId"];
    let query = { isDeleted: false, brokerId: pararms.brokerId };
    if (pararms.search) {
      query['$or'] = [
    ...
    data = await utils.getData(Visit, {
      query: query,
      sort: { _id: -1 },
      pageSize: pararms.pageSize,
      pageNo: pararms.pageNo,
    ...
    for (let datum of data) {
      result.push({
        ...
        status: datum.visitStatus
      });
    }
  }
  else if (pararms.type == 'loan') {
    const populates = ['dsaId']
    let query = { isDeleted: false };
    if (pararms.search) {
      query['$or'] = [
        { _id: { $regex: pararms.search, $options: "i" } },
        { name: { $regex: pararms.search, $options: "i" } },
        { createdAt: { $regex: pararms.search, $options: "i" } },
        { queryStatus: { $regex: pararms.search, $options: "i" } },
      ];
    }
    data = await utils.getData(LoanQueryDetails, {
      query: query,
      populates,
    });
    for (let datum of data) {
      result.push({
        id: datum._id,
        name: datum.dsaId.companyName,
        date: datum.createdAt,
        status: datum.queryStatus
      });
    }
  }
  else if (pararms.type == 'claim') {
    const populates = ["brokerId", "builderId", {
    ...
    const query = { brokerId: pararms.brokerId, isDeleted: false };
    if (pararms.search) {
      query['$or'] = [
        { _id: { $regex: pararms.search, $options: "i" } },
        { date: { $regex: pararms.search, $options: "i" } },
        { claimStatus: { $regex: pararms.search, $options: "i" } },
        { brokerageAmount: { $regex: pararms.search, $options: "i" } },
      ];
    }
    data = await utils.getData(Claim, {
      query: query,
      sort: { _id: -1 },
      populates,
    });
    for (let datum of data) {
      result.push({
        id: datum._id,
        date: datum.date,
        status: datum.claimStatus,
        amount: datum.brokerageAmount
      });
    }

  }
  else if (pararms.type == 'referral') {

    result = [{
      brokerId:"642428a0ab5cba4f0ce5a35b",
      mobileNo:"1111166666",
      date:"11/12/2023",
      amount:"2000"
    }]
